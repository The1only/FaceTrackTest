...
TargetDetector::TargetDetector() : confidence_threshold_(50.5), input_image_height_(300), input_image_width_(300),
                               scale_factor_(1.0), mean_values_({104., 177.0, 123.0}) {

    network_ = cv::dnn::readNetFromCaffe(FACE_CONFIGURATION, FACE_WEIGHTS);

    if (network_.empty()) {
        std::ostringstream ss;
        ss << "Error loading network with the following settings:\n"
           << "Configuration: " + std::string(FACE_CONFIGURATION) + "\n"
           << "Binary: " + std::string(FACE_WEIGHTS) + "\n";
        throw std::invalid_argument(ss.str());
    }
}

// Deep Learning detector....
std::vector<cv::Rect> TargetDetector::detect_target_rectangles(const cv::Mat &frame) {
